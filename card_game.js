...
function buildDeck() {
  ...
  for (let i = 0; i < ranks.length; i++) {
    for (let j = 0; j < suits.length; j++) {
      ...
      // Obj[rank] = rank[j];
      // Obj[value] = j;
      deck.push({ suit: suits[j], rank: ranks[i], value: j });
    }
  }
  return deck;
}
...
const shuffle = (deck) => {
  ...
  while (currentIndex !== 0) {
    // console.log(shuffleDeck[currentIndex]);
    ...
    //every time it swap the value between shuffleDeck[currentIndex] and shuffleDeck[randomIndex]
...
const greet = () => {
  let name = getInput("welcome to the card game, what your name");
  console.log(name);
  return name;
};
...
const guess = (takeFirstCard, takeSecondCard) => {
  // console.log(takeFirstCard + "diyizhangka" + takeFirstCard[rank]);
  console.log(`rank: ${takeFirstCard.rank}, suit: ${takeFirstCard.suit}`);
  let input = getInput(
    `Do you think the next card will be higher (h) or lower (l) then your current card ?`
  );
  if (input === "h") {
    return compare(takeFirstCard, takeSecondCard) < 0;
  } else if (input === "l") {
    return compare(takeFirstCard, takeSecondCard) > 0;
  } else {
    console.log(
      "please guess either h or l as a input you got no points for this round"
    );
    return false;
  }
};
...
const playGame = () => {
  ...
  while (score < 5 && score < deck.length) {
    ...
    if (guess(currentCard, nextCard)) {
      score++;
      console.log(`congratulate your score is ${score}`);
    } else `You were wrong and you got no points for this round`;
    currentCard = nextCard;
  }

  deck.length === 0
    ? console.log("you're out of cards and you lost")
